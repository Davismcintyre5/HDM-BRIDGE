...
const createApiKey = async (req, res, next) => {
  try {
    ...
    logger.info(`API key created: ${apiKey.name} for org ${req.organizationId}`);
...
const revokeApiKey = async (req, res, next) => {
  try {
    ...
    if (!apiKey) {
      return next(new AppError('API key not found', 404, 'NOT_FOUND'));
    }
